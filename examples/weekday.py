...
async def main():
    ...
    try:
        ...
        result = await intratools.mobile.classroom.reserve_class(session, 1, "A204", "pjc2317", "21073")
        if result['resultType'] == 'ok':
            msg += "평일 1타임 - 성공! (일반)"
        else:
            msg += f"평일 1타임 - 실패! ({result['resultMsg']})"

        result = await intratools.mobile.classroom.reserve_class(session, 4, "A204", "dorm", "21073")
        if result['resultType'] == 'ok':
            msg += "\n평일 1타임 - 성공! (일반)"
        else:
